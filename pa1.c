...
void sigterm_handler(int signo){
	...
	sigaction(SIGTERM, &oldact_sigalrm, NULL);
	kill(getpid(), SIGKILL);
}
...
static int run_command(int nr_tokens, char *tokens[])
{
	...
	newact_sigterm.sa_handler = sigterm_handler;

	sigaction(SIGALRM, &newact_sigalrm, &oldact_sigalrm);
	sigaction(SIGTERM, &newact_sigterm, &oldact_sigterm);
